...
export default function Home(): JSX.Element {
  // ================= 配置区 ================= 
  const CONFIG_FEEDBACK: 'FIXED' | 'RANDOM' = 'RANDOM'; // 'FIXED' = 100% 组, 'RANDOM' = 60% 组 (随机顺序，但固定6次脸红)
  ...
  const triggerBlushEffect = () => {
    ...
    if (shouldBlush === undefined) return;
  ...
  useEffect(() => {
    if (conversationEnded) {
      // Use a timeout to ensure state is fully flushed (optional safety)
      setTimeout(() => {
          const dataPayload = JSON.stringify(conversationHistory);
          console.log("【Vercel】Final Send:", conversationHistory); // Debug log
  ...
  }, [conversationEnded, conversationHistory]);
  ...
  const sendMessage = async (): Promise<void> => {
    ...
    setConversationHistory(prev => [...prev, userMessageRecord]);
      ...
      setTimeout(() => {
        if (data.success) {
          ...
          const aiResponseRecord = {
            ...
            action_type: "text",
            content: reply,
            ai_response: shouldBlush ? "blush" : "normal",
            timestamp: new Date().toISOString()
          };
          setConversationHistory(prev => [...prev, aiResponseRecord]);
          
          setMessages((prev: Message[]) => [...prev, { sender: 'bot', content: reply, shouldBlush, showHearts, isEmotional }]);
        } else {
          const errorMessage = 'Connection error: Unable to reach the server. Please check your network connection and try again.';
          // 记录AI错误回复
          const aiErrorRecord = {
            round: currentRound + 1,
            role: "ai",
          ...
          setConversationHistory(prev => [...prev, aiErrorRecord]);
  ...
  const handleActionClick = async (action: string): Promise<void> => {
    ...
    setConversationHistory(prev => [...prev, buttonClickRecord]);
    ...
    <div className={styles.deviceContainer}>
      <div className={styles.deviceFrame} id="experimentApp">
        ...
        <div className={styles.rightPanel}>
          <div className={styles.chatHistory} id="chatHistory" ref={chatHistoryRef}>
          ...
          <div className={styles.chatInputArea}>
            {conversationEnded ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px', width: '100%', padding: '20px' }}>
                ...
                <p style={{ margin: '10px 0 0 0', fontWeight: 'bold', color: '#3b82f6' }}>Please proceed to the next page</p>
              </div>
            ) : (
              <>
                <input
